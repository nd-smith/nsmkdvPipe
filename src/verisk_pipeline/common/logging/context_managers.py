...
from verisk_pipeline.common.logging.context import get_log_context, set_log_context
from verisk_pipeline.common.logging.utilities import log_exception, log_with_context


def _extract_instance_context(instance: Any) -> Dict[str, Any]:
    """Extract table name and other context from instance attributes."""
    context: Dict[str, Any] = {}

    if hasattr(instance, "table_path"):
        path = instance.table_path
        # Extract table name: abfss://.../.../Tables/events -> "events"
        if "/" in path:
            context["table"] = path.rstrip("/").split("/")[-1]
        else:
            context["table"] = path

    if hasattr(instance, "primary_keys") and instance.primary_keys:
        context["primary_keys"] = instance.primary_keys
