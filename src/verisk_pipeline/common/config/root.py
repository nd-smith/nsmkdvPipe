...
from verisk_pipeline.common.config.claimx import (
    ClaimXConfig,
    load_claimx_config_from_dict,
)
...
class RootConfig:
    ...
    xact: Optional[PipelineConfig] = None
    claimx: Optional[ClaimXConfig] = None
    ...
    def enabled_domains(self) -> List[str]:
        ...
        if self.xact is not None:
            domains.append("xact")
        if self.claimx is not None:
            domains.append("claimx")
        return domains
...
def load_root_config(
    ...
    # Build claimx config if present
    claimx_config = None
    if "claimx" in data and data["claimx"] is not None:
        claimx_data = data["claimx"].copy()
        # Inherit shared settings if not specified in domain
        for key in ["health", "logging", "observability", "security"]:
            if key not in claimx_data and key in data:
                claimx_data[key] = data[key]
        claimx_config = load_claimx_config_from_dict(claimx_data)
...
# Module-level cached root config
