...
from verisk_pipeline.claimx.api_client import ClaimXApiError
from verisk_pipeline.claimx.stages.handlers.utils import elapsed_ms
...
def with_api_error_handling(
    api_calls: int = 1,
    log_context: Callable[["ClaimXEvent"], dict] | None = None,
) -> Callable[
    [Callable[..., Awaitable["EnrichmentResult"]]],
    Callable[[Any, "ClaimXEvent"], Awaitable["EnrichmentResult"]],
]:
    """
    ...
    def decorator(
        ...
        async def wrapper(self: Any, event: "ClaimXEvent") -> "EnrichmentResult":
...
class EventHandler(ABC):
    ...
    def __init__(self, client: ClaimXApiClient):
        self.client = client
    ...
    @abstractmethod
    async def handle_event(self, event: ClaimXEvent) -> EnrichmentResult:
        """
    ...
    async def handle_batch(self, events: List[ClaimXEvent]) -> List[EnrichmentResult]:
        """
    ...
    async def process(self, events: List[ClaimXEvent]) -> HandlerResult:
        """
    ...
    async def _process_batched(
        self, events: List[ClaimXEvent]
    ) -> List[EnrichmentResult]:
        """Process events in batches grouped by batch_key, concurrently."""
        import asyncio

        # Group by batch key
        groups: Dict[Any, List[ClaimXEvent]] = defaultdict(list)
        for event in events:
        ...
        async def process_group(key: Any, group_events: List[ClaimXEvent]):
        ...
        for result in results:
            ...
            log_entry = {
                ...
                "status": (
                    "success"
                    if result.success
                    else ("failed_permanent" if not result.is_retryable else "failed")
                ),
                ...
                "processed_date": now.date().isoformat(),  # ADD: Date partition column
            }
...
class NoOpHandler(EventHandler):
    ...
    Extracts data directly from the Kusto event payload.
    """
    ...
    def extract_rows(self, event: ClaimXEvent) -> EntityRows:
        ...
        Returns:
            EntityRows extracted from payload
        """
        pass
...
class HandlerRegistry:
    ...
    def get_handler_class(self, event_type: str) -> Optional[Type[EventHandler]]:
    ...
    ) -> Dict[Type[EventHandler], List[ClaimXEvent]]:
        """
        Group events by their handler class.
        """
        groups: Dict[Type[EventHandler], List[ClaimXEvent]] = defaultdict(list)
