...
class TaskTransformer:
    ...
    API response structure (from /customTasks/assignment/{assignmentId}):
    {
        ...
        "dateCompleted": "...",
        "cancelledDate": "...",
        "cancelledByResourceId": 333,
        "status": "COMPLETED",
        "pdfProjectMediaId": 444,
        "dateExported": "...",
        "formResponseId": "...",
        "stpEnabled": true,
        "stpStartedDate": "...",
        ...
        "customTask": {
            "taskId": 456,
            "compId": 555,               # Note: compId not companyId
            "name": "...",
            "description": "...",
            "formId": "...",
            "formName": "...",
            "enabled": true,
            "default": true,             # Note: "default" not "isDefault"
            "isManualDelivery": false,
            "isExternalLinkDelivery": true,
            "providePortalAccess": false,
            "notifyAssignedSendRecipient": true,
            "notifyAssignedSendRecipientSms": true,
            "notifyAssignedSubject": "...",
            "notifyTaskCompleted": true,
            "notifyCompletedSubject": "...",
            "allowReSubmit": true,       # Note: capital S
    ...
    def to_task_row(
        ...
        Args:
            data: Task assignment dict from API
    ...
    def to_template_row(
        ...
        Args:
            template: customTask dict from API
            source_event_id: Event ID for traceability
    ...
    def to_link_row(
        ...
        Args:
            link: externalLinkData dict from API
            assignment_id: Task assignment ID
            project_id: Project ID
            source_event_id: Event ID for traceability

        Returns:
            External link row dict
        """
        now = now_iso()
...
class TaskHandler(EventHandler):
    ...
    async def handle_event(self, event: ClaimXEvent, start_time: datetime) -> EnrichmentResult:  # type: ignore[override]
        ...
        response = await self.client.get_custom_task(event.task_assignment_id)
        ...
        project_id = safe_int(response.get("projectId")) or event.project_id
        assignment_id = (
            safe_int(response.get("assignmentId")) or event.task_assignment_id
        )
