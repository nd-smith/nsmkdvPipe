...
from verisk_pipeline.common.exceptions import ErrorCategory, PermanentError
...
class ProjectHandler(EventHandler):
    ...
    async def handle_event(self, event: ClaimXEvent) -> EnrichmentResult:
        ...
        try:
            ...
            api_start = time.time()
            response = await self.client.get_project(event.project_id)
            ...
            log_with_context(
                ...
                transform_duration_ms=round(transform_duration_ms, 2),
    ...
    async def ensure_exists(
        ...
        project_id: str,
        cache: ProjectCache,
    ) -> Tuple[bool, EntityRows, int]:
        """
        Ensure project exists in database. Fetches from API if not in cache.
        ...
        Returns:
            Tuple of (success, entity_rows, api_calls)
        """
        if cache.exists(project_id):
            log_with_context(
                logger,
                logging.DEBUG,
                "Project exists in cache",
                project_id=project_id,
            )
            return True, EntityRows(), 0

        log_with_context(
            logger,
            logging.DEBUG,
            "Project not in cache, fetching from API",
            project_id=project_id,
        )
        ...
        try:
            ...
            log_with_context(
                logger,
                logging.DEBUG,
                "Fetched missing project",
                project_id=project_id,
                projects_count=len(rows.projects),
                contacts_count=len(contact_rows),
                api_duration_ms=round(api_duration_ms, 2),
            )
            ...
            log_with_context(
                logger,
                logging.WARNING,
                "Project not found (permanent error)",
                project_id=project_id,
                error_message=str(e)[:200],
                error_category="permanent",
                api_duration_ms=round(api_duration_ms, 2),
            )
            return False, EntityRows(), 1
...
class ProjectTransformer:
    ...
    def to_project_row(
        ...
        # Log transformation details at trace level
        log_with_context(
            logger,
            logging.DEBUG,
    ...
    def to_contact_rows(
        ...
        contacts = []
        now = now_iso()
        today = datetime.now(timezone.utc).date().isoformat()  # ADD: Date partition
            ...
            contacts.append(
                {
                    ...
                    "created_at": now,
                    "updated_at": now,
                    "created_date": today,  # ADD: Date partition column
