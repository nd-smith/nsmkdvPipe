"""
Retry handler for xact domain download tasks.
...
(FailedDownloadMessage, DownloadTaskMessage) for retry and DLQ routing.
"""
...
class DownloadRetryHandler:
    """
    Handles retry logic for download tasks via Kafka topics with exponential backoff.
    ...
    Usage:
        ...
        >>> retry_handler = DownloadRetryHandler(config, producer)
        >>>
    ...
    def __init__(
        ...
        logger.info(
            "Initialized DownloadRetryHandler",
            extra={
                "domain": domain,
...
RetryHandler = DownloadRetryHandler


__all__ = [
    "DownloadRetryHandler",
    "RetryHandler",  # Backwards compatibility alias
]
