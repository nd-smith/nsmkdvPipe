...
def print_report(results: dict) -> None:
    ...
    if results["duplicate_rows"] > 0:
        ...
        if dup_pct > 10:
            ...
            print("   2. Deduplication cache not initialized on poller start")
            print("   3. Poller restarted without loading existing trace_ids from Delta")
            print("   4. Multiple poller instances running simultaneously")
            print("")
            print("   Recommended actions:")
            print("   1. Check poller logs for 'Initialized trace_id cache' message")
            print("   2. Verify dedup cache size matches expected trace_id count")
            print("   3. Check for multiple poller pods/processes")
        elif dup_pct > 1:
            print("⚠️  Moderate duplicate rate detected.")
            print("   This may be expected due to the 5-minute overlap window.")
