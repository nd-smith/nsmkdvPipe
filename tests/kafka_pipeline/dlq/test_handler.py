...
class TestDLQHandlerInit:
    ...
    def test_init_with_config(self, kafka_config):
        ...
        handler = DLQHandler(kafka_config)
...
class TestDLQHandlerStartStop:
    ...
    @pytest.mark.asyncio
    @patch("kafka_pipeline.dlq.handler.BaseKafkaProducer")
    @patch("kafka_pipeline.dlq.handler.BaseKafkaConsumer")
    async def test_start_creates_producer_and_consumer(
        ...
        try:
            ...
            assert call_kwargs["topics"] == [kafka_config.dlq_topic]
            assert call_kwargs["group_id"] == "test-dlq-handler-worker"
...
class TestDLQHandlerReplay:
    ...
    async def test_replay_message_success(
        ...
        assert call_kwargs["topic"] == kafka_config.downloads_pending_topic
