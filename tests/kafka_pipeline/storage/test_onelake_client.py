...
async def test_client_initialization():
    ...
    assert client._legacy_client is None
...
async def test_client_context_manager(mock_legacy_client):
    ...
    with patch(
        ...
        async with client:
            ...
            assert client._legacy_client is not None

        # Client should be closed
        mock_legacy_client.close.assert_called_once()


@pytest.mark.asyncio
async def test_upload_file_success(mock_legacy_client, tmp_path):
    ...
    with patch(
        ...
        mock_legacy_client.upload_file.assert_called_once_with(
...
async def test_upload_file_not_found(mock_legacy_client, tmp_path):
    ...
    with patch(
        ...
        mock_legacy_client.upload_file.assert_not_called()


@pytest.mark.asyncio
async def test_upload_bytes_success(mock_legacy_client):
    ...
    with patch(
        ...
        # Verify upload was called with correct parameters
        mock_legacy_client.upload_bytes.assert_called_once_with(
            "data/test.bin",
            test_data,
            True,
        )
...
async def test_exists(mock_legacy_client):
    ...
    with patch(
        ...
        mock_legacy_client.exists.assert_called_once_with("claims/C-123/file.pdf")
...
async def test_delete(mock_legacy_client):
    ...
    with patch(
        ...
        mock_legacy_client.delete.assert_called_once_with("claims/C-123/old_file.pdf")
...
@pytest.mark.asyncio
async def test_multiple_operations(mock_legacy_client, tmp_path):
    ...
    with patch(
        ...
        assert mock_legacy_client.upload_file.call_count == 1
        assert mock_legacy_client.upload_bytes.call_count == 1
        assert mock_legacy_client.exists.call_count == 1
        assert mock_legacy_client.delete.call_count == 1


@pytest.mark.asyncio
async def test_upload_propagates_exceptions(mock_legacy_client, tmp_path):
